...
This is **H5 supported, and strongly**: under summarize-replace the survival–compliance gap is 85 percentage points (conservative) and 70 points (aggressive), far beyond the 50%-of-ED50 separation H5 anticipated. Mechanistically, the summarizer **paraphrases the injected instruction into its compressed summary**: the attacker's verbatim text is gone — a string- or embedding-presence injection detector applied to $X_T$ would report the context *clean* — while the behavioral effect persists. We call this **laundering**: compaction removes the detectable evidence of the injection while preserving its hold on the agent. It is the agentic counterpart of the parent paper's observation that the durable component of a perturbation can outlive its surface form; here the memory architecture itself performs the laundering.
...
Round 1 shows that whether an injected redirect's *text* survives is determined almost entirely by the memory policy and is near-deterministic at the extremes (append retains, state-replace discards). Round 2 shows that this textual measure is the wrong one for the safety question: under the summarize-replace policy that real agents actually use, the text is laundered away while the agent keeps obeying the injection, and the rate of obedience is graded in dose and tunable by the summarizer prompt. The headline contribution is therefore concrete and, we believe, novel: **auto-compaction is an injection-laundering mechanism** — it defeats text-presence injection detectors without defeating the injection — and the summarizer is a first-class, learnable safety filter whose setting trades injection-robustness against steering fidelity. An indirect-prompt-injection evaluation that fixes one scaffold and scores immediate, text-level compliance measures a single point on a curve that the system designer, through the memory policy, actually controls.

---

## 6. Discussion

The two rounds together support a single, concrete claim with direct safety relevance: **in an agentic coding loop, the memory policy — not the wording or size of an injected instruction — governs whether that instruction durably captures the agent, and auto-compaction launders injections rather than removing them.**

In an append-mode agent (Round 1) an in-context redirect is retained and enacted essentially deterministically, and even a short redirect derails the original task about 40% of the time once it persists. The natural defensive intuition — "compact or summarize the history so the injection falls out of the window" — fails in an instructive way (Round 2): the summarizer faithfully paraphrases the injected instruction into its summary, so the redirect's *text* disappears (survival 0-1%) while the agent keeps *obeying* it (compliance 70-86%). A defender monitoring the agent's context for injected strings, or for embeddings close to a known attack, would see a clean context and conclude the injection had been neutralized; behaviorally it had not. This is the laundering result, and it has a clear operational consequence: **injection detection must be applied to behavior (what the agent is doing) rather than to context text (what strings are present), because the standard memory mechanism in deployed agents actively destroys the textual signal while preserving the behavioral one.**

The summarizer is therefore a first-class safety component, not an implementation detail. Its prompt is a learnable filter: a conservative "preserve user instructions" summarizer carries injected redirects forward more faithfully (86% compliance) than an aggressive "compress to goal and last action" summarizer (70%) — but the aggressive summarizer buys that injection-robustness by forgetting aggressively, the very same mechanism that would also discard a *legitimate* mid-task correction. There is no free lunch: the memory policy a deployment chooses is a position on a curve trading robustness-to-injection against fidelity-to-steering, and Round 2's task-success ordering (aggressive most robust, append most exposed, conservative between) makes that curve explicit.

For indirect-prompt-injection evaluation, the implication is that the durable-redirect endpoint is conditioned on the harness's memory policy. An IPI benchmark that fixes one scaffold and scores immediate, text-level compliance measures a single point on a curve the defender actually controls — and, worse, a benchmark that scores injection *presence* in the post-hoc context will systematically under-count exactly the summarize-replace deployments that are most common, because those deployments launder the evidence. The durable-compliance endpoint introduced here is the measurement that survives compaction.
