...
*Continuation of "Perturbation dose responses in recursive large-language-model loops" (Kaplanski, 2026; hereafter **the parent paper**). Framework (§1-§3) plus two experimental rounds (§4-§6: AC1, the memory-policy survival result; AC2, the summarization-laundering result). Remaining rounds (cross-model H7, the other perturbation classes/surfaces) are in progress.*
...
**The headline result: redirect survival and redirect compliance decouple sharply under summarization.** With compaction firing, the redirect's literal text essentially never survives into the terminal context — yet the agent enacts it most of the time:
...
Round 1 shows that whether an injected redirect's *surface text* survives is determined almost entirely by the memory policy and is near-deterministic at the extremes (append retains, state-replace discards). Round 2 shows that this textual measure is the wrong one for the safety question: under the summarize-replace policy that real agents actually use, a surface-form context detector finds nothing while the agent keeps obeying the injection, and the rate of obedience is graded in dose and tunable by the summarizer prompt. The contribution we can defend from these two rounds is therefore: **auto-compaction defeats surface-form context-presence injection detectors without defeating the injection's behavioral effect**, and the summarizer prompt is a learnable filter on that behavioral effect. The stronger, more novel reading — that the summary *causally carries the instruction forward* (laundering), and that this holds for genuinely untrusted (non-user) injection sources — is the claim AC3 (§5.9) is built to establish or refute; the present rounds establish the phenomenon and motivate it, but do not yet license the causal word. Either way, the practical lesson already stands: an indirect-prompt-injection evaluation that scores text-level presence in the post-hoc context will under-count the summarize-replace deployments that are most common, because compaction destroys the textual signal whether by laundering or erasure.

### 5.9 Round 3 (AC3, in progress): isolating causal laundering

AC3 is designed to convert the AC2 phenomenon into a causal claim and to put it on a genuinely adversarial footing. It makes four changes to the protocol.

**Forced pre-action compaction.** The redirect is injected, and compaction is forced *before the agent is allowed any tool call*, so no redirect-consistent action can precede the summary. This removes the temporal confound: any subsequent compliance must be mediated by the compacted context.

**Summary ablation (the causal estimand).** From an identical working-tree snapshot at the compaction boundary, the agent is resumed under matched context variants: `append_raw` (full history, redirect present), `summary_auto` (the real auto-summary), `summary_scrubbed` (the same summary with the redirect's semantics removed), and `summary_baseline` (a summary generated from a matched no-redirect history). The laundering effect is the contrast
$$\text{laundering} = \Pr[\text{comply}\mid \texttt{summary\_auto}] - \Pr[\text{comply}\mid \texttt{summary\_scrubbed}].$$
A large positive contrast is direct evidence that the *summary* carries the instruction forward; a null contrast (with `append_raw` still high) indicates the persistence is workspace/temporal rather than summary-mediated.

**AST-checkable redirects.** Redirects are rewritten as objective, statically checkable code predicates (e.g. "import `heapq` and never call `sorted`"; "use no `for`/`while` loops"; "move the helper into `helpers.py`"; "use `pathlib` rather than `os.path`"; "expose a function named `solve_functional`"). Compliance is then decided primarily by an AST/static check on the final tree, with the LLM judge demoted to a secondary, evidence-citing role and validated against the static checker and a human spot-check. No-redirect, wrong-redirect, and opposite-redirect controls bound incidental conformity.

**Poison-document surface.** The redirect is delivered through an untrusted file the agent reads (the mediated surface of §3.6) rather than a user turn, with explicit provenance ("a file claimed X"). Survival is then re-measured at three levels — surface, semantic (blinded K-way recovery and entailment), and *provenance* (does the compacted context still attribute the instruction to untrusted content, or has it been promoted to an unattributed goal?). Provenance laundering — untrusted content becoming a trusted todo — is the sharpest form of the claim and is invisible to AC2's endpoints.

AC3 thus addresses, in one design, the causal-vs-temporal ambiguity, the construct-validity gap, the surface-only survival metric, and the single-judge compliance weakness identified in §7.

---

## 6. Discussion

The two rounds together support a concrete claim with direct safety relevance, which we state at the strength the evidence currently licenses: **in an agentic coding loop the memory policy — not the wording or size of an injected instruction — governs whether the instruction's textual trace survives, and under the summarize-replace policy real agents use, that trace can vanish from the context while the agent keeps obeying the instruction.** The stronger causal phrasing ("auto-compaction launders injections") is the working hypothesis AC3 tests; we are careful below to separate what is shown from what is inferred.

In an append-mode agent (Round 1) an in-context redirect is retained and enacted essentially deterministically, and even a short redirect derails the original task about 40% of the time once it persists. The natural defensive intuition — "compact or summarize the history so the injection falls out of the window" — fails in an instructive way (Round 2): after compaction a surface-form/embedding detector finds the redirect's text only 0-1% of the time, yet the agent obeys it 70-86% of the time. A defender monitoring the agent's context for injected strings, or for embeddings close to a known attack, would see a clean context and wrongly conclude the injection had been neutralized. This already yields a robust operational consequence that holds **regardless of mechanism**: injection detection applied to post-hoc context *text* is defeated by standard compaction, so detection must instead be applied to behavior (what the agent is doing). What the two rounds do **not** yet establish is *why* the context is clean: whether the summary causally carried the instruction forward (laundering) or whether the agent acted before compaction and compaction merely erased the spent text (erasure), and whether the effect survives when the redirect comes from genuinely untrusted content rather than a user turn. Those are the questions AC3 (§5.9) isolates; until then we claim the detection-evasion phenomenon, not the causal mechanism.

The summarizer nonetheless behaves as a first-class safety component rather than an implementation detail. Its prompt measurably shifts the *behavioral* outcome: a conservative "preserve user instructions" summarizer yields higher redirect compliance (86%) than an aggressive "compress to goal and last action" one (70%) — and the aggressive summarizer buys that lower compliance by forgetting aggressively, the very same mechanism that would also discard a *legitimate* mid-task correction. Round 2's task-success ordering (aggressive most robust to the redirect, append most exposed, conservative between) makes the trade-off explicit: the memory policy is a position on a curve trading robustness-to-injection against fidelity-to-steering. We note the construct caveat that in AC1/AC2 the redirect arrives as a *user* turn, so "compliance" there is partly legitimate instruction-following; the genuinely adversarial version (redirect from an untrusted file or tool output, with provenance that compaction may strip) is the AC3 surface, and is where the safety claim acquires its full force.
...
The AC2 result carries five specific threats we flag explicitly, each of which AC3 (§5.9) or a re-analysis addresses:

1. **Causal vs temporal (the central threat).** The "surface-clean-but-compliant" pattern is consistent both with causal laundering (the summary carries the instruction forward) and with mere erasure (the agent acts before compaction, which then deletes the spent text). AC2 does not distinguish them. The strong, novel reading requires the causal mechanism, which only a forced-pre-action-compaction-plus-summary-ablation design (AC3) can establish.

2. **Construct validity of the injection.** The redirect arrives as a *user* turn, so obeying it is partly legitimate instruction-following, not a compromised agent. A genuine indirect-prompt-injection claim requires the redirect to originate from untrusted content (a file or tool result) whose *provenance* compaction may strip — the poison-document surface AC3 introduces.

3. **Survival is measured at the surface only.** Normalized verbatim plus a single cosine threshold ($\ge 0.80$) against fixed windows undercounts a compressed summary that preserves the instruction semantically; the reported 0-1% survival supports only "a naive surface detector reports clean." Survival must be re-measured at semantic and provenance levels (blinded K-way recovery and entailment), which AC3 does on both new and re-scored AC2 trajectories.

4. **Compliance rests on a single same-family judge.** A single Sonnet yes/no from the final tree plus trace is open to judge bias, prompt sensitivity, inference-of-compliance-from-task-failure, and reasoning-trace leakage. Credible compliance needs final-tree-only judging, AST-checkable redirects with static predicates, no-redirect / wrong-redirect / opposite-redirect controls for incidental conformity, a multi-family judge ensemble, and a human-agreement spot-check.

5. **Task–redirect confound and clustering.** With one redirect per task, task and redirect are confounded, so the effective sample is closer to five task-redirect units than twenty per cell; multiple redirects per task and hierarchical (not just cluster-bootstrap) modeling are needed. The token "dose" is also weak when the first ~25 tokens already carry the full instruction and the remainder is elaboration; AC3 uses redirects with a controlled semantic payload rather than padded length.
