...
def test_sharpness_dim_fractional():
    ...
    # j* candidates: i=1 → cumsum=0.5 ≥ 0 ok; i=2 → cumsum=0.3 ≥ 0 ok;
...
def test_effective_rank_counts_nonnegative():
    ...
    # Everything >= -0.01 counts: 0.1, 0.01, -0.02 is below threshold=-0.01? we use 0.01 threshold
    # items with value >= -0.01 → 0.1, 0.01 → 2
    assert rk == 2
