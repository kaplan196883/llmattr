...
tested dose up to 400 tokens. Replace-mode loops show
near-saturated raw switching under the default perturbation, but this
mostly reflects *state-reset overwrite*: the injected text becomes
\(X_{t+1}\), so switching is tautological at the state-write step.
Insert-mode probes decouple model exposure from state-write and
reduce replace-mode switching to 12-32%; the F3 cross-loop check
shows a smaller append-mode overwrite-insert gap (14-34 pp versus
60-80 pp in replace-mode), with insert rates ordered
\(O1 \ge O2 \ge O3\).
...
- **Token.** A small chunk of text the LLM works with - often a word
  ...
  the headline result of ≈ 40 tokens is about a short paragraph.
...
- **ED50.** Half-effect dose, borrowed from pharmacology: the dose
  ...
  the dose at which raw switching crosses 50%;
  $\mathrm{ED50}_{\mathrm{persist}}$ is the dose at which persistent
  escape crosses 50%.
...
But durable change is not achieved even at 400 tokens. Raw
end-state disagreement rises and plateaus near 67%, while two
*identical* unperturbed runs already disagree about 35% of the time
from random sampling alone - the **stochastic floor** - so the real
effect of the injection saturates around +32 percentage points
above noise. The strict endpoint - knocked into a new end-state and
still there at the end of the run - never crosses 50% in the tested
5-400 token range. **Replace-mode** loops (each step rewrites the
...
- **About 40 tokens of injected text flip the end-state in roughly
  half of runs** ($\mathrm{ED50}_{\mathrm{raw}} \approx 40$). This is
  the half-effect dose for the simplest "did it move?" question, in
  adversarial-content append-mode.
- **Two identical runs already disagree about 35% of the time from
  random sampling alone** (the *stochastic floor*). Any reported
  switching rate has to be measured *above* this baseline, otherwise
  ordinary randomness is miscounted as successful redirection.
- **Durable change is never reached in the tested 5-400 token
  range** (no $\mathrm{ED50}_{\mathrm{persist}}$). At the largest
  injection, 16% of runs still looked changed when we judged the
  recent conversation window, and 27% when we judged only the
  final reply. Even the more permissive final-reply test is far
  below the 50% half-effect threshold. The largest estimate seen
  in any observable check, at any tested dose, was 31%; see §5.1.1
  for the observable-by-observable breakdown.
- **Replace-mode "fragility" is mostly the update rule, not the
  model.** When we hold the injection fixed but compare overwriting
  the context against inserting into it, real flip rates drop to
  12-32%. Most of the apparent fragility is mechanics, not model
  weakness.
...
4. **Do not assume small adversarial nudges permanently redirect
   ...
   produce 50% persistent escape up to 400 tokens. For jailbreak
   or agent-redirection evaluations, this means the meaningful
   target is sustained post-attack behavior, not a one-step
   perturbation success.

5. **Evaluating indirect prompt injection (IPI).** Most IPI
   benchmarks (standardized tests for comparing defenses) today
   report whether the model follows the injected instruction at
   the next step - that is the *raw-switching* endpoint. A defense
...
(rerun on models from other companies). The tested perturbation
range is 5-400 tokens, so persistent escape may appear at higher
doses. Our main "end-state" measurement looks at the last 4000
characters of the running conversation, roughly the last 6-7
generations. This can dilute one-off changes confined to the final
reply. When we instead judge only the final reply, persistence
rises to 27% at the largest dose but still remains far below 50%
(§5.1.1). Crucial scope note: the loop we study is a *bounded-
memory* recursive loop (the running conversation is clipped to
~12,000 characters - well below the model's native ~128,000-
token capacity). This bounded memory is part of the experiment, not
an artifact: real recursive agents use rolling windows, summary
buffers, retrieved-document memory, and other limited-memory
policies. Findings should be read as describing this bounded loop;
a loop with much more memory could behave differently. We report a
buffer-size sensitivity check in §5.1.3. The regimes are
...
![Fig 1. **Headline perturbation dose response (append + dialog only).** Summary dose-response view for recursive-loop perturbations in append-mode O1 and dialog D1. Replace-mode regimes O2/O3 are intentionally excluded: under the default state-reset overwrite protocol, \(X_{t+1}=\operatorname{clip}(\text{perturbation\_text})\), so raw switching is tautological at the state-write step (see §5.2 and Fig 5). The figure emphasizes that raw switching, stochastic floor, and persistent escape are distinct endpoints, and orients the reader before the formal endpoint definitions. Source: `data/aggregated/perturbation_dose_response/dose_response.png`.[^Fig1]](data/aggregated/perturbation_dose_response/dose_response.png)
...
![Fig 4. **Persistent escape under cluster granularity.** Persistent-escape rates recomputed under K-means $k=12$, K-means $k=4$, and HDBSCAN, all on the canonical `context_tail` observable. No clustering convention reaches the 50% persistent-escape threshold up to 400 injected tokens. Observable-robustness of the same conclusion is checked in §5.1.1: under `output` (single-step) the rate rises to 27% at dose 400, still below 50%. Source: `data/aggregated/multi_granularity_persistence.png`.[^Fig4]](data/aggregated/multi_granularity_persistence.png)
...
#### 5.1.2 Saturation under the canonical bounded-memory policy

A natural follow-up is whether persistent escape would simply require more tokens than 400. We fit a 4-parameter logistic to each observable's persistence dose-response and read off the upper asymptote.

| observable | ceiling (asymptotic plateau) | midpoint dose (inflection) | extrapolated $\mathrm{ED50}_{\mathrm{persist}}$ |
|---|---:|---:|---|
| `output` (single-step) | 0.31 ± 0.03 | ~52 tokens | not reached at any dose |
| `rolling_k3` (3-step window) | 0.18 ± 0.02 | ~107 tokens | not reached at any dose |
| `context_tail` (canonical) | 0.15 ± 0.02 | ~139 tokens | not reached at any dose |

Under the 4PL fit, all three observables predict an asymptotic plateau well below the 50% half-effect threshold; the upper-95% bound on the ceiling is approximately 0.37 under `output` and tighter under the other two observables. The midpoints (52, 107, 139 tokens) sit inside the tested 5-400 range, so the curves have effectively reached their plateaus by ~200-300 tokens. Plot: `data/aggregated/multi_observable_persistence/extrapolation.png`.

**Important scope condition.** This extrapolation describes the bounded-memory loop tested here: gpt-4o-mini under append-mode update with `max_context_chars = 12000` (tail-clip), `context_tail_chars = 4000`. The clip is part of the experimental object, not an artifact: real recursive systems use rolling windows, summary buffers, retrieval-augmented memory, and other bounded-memory policies. Within this loop, the dose response saturates below 50%. The result is *not* a claim about all recursive LLM systems. In particular, a loop with the same generator and a much larger memory buffer (or no clipping below the model's native 128K context) might have a different persistence ceiling. We characterize this dependency explicitly in §5.1.3 by re-running O1 at a buffer size that does not clip during the experiment.

The 4PL extrapolation has further caveats inside the canonical loop: (a) the 4PL is a saturating functional form; a different model class could behave differently. (b) Doses beyond ~3000 tokens (12000 chars / 4 chars-per-token) overflow the max_context buffer and the experiment then transitions out of the genuine append-mode regime into a state-replacement regime that is structurally similar to §5.2's replace-mode tautology. So extrapolation past ~3000 tokens is meaningless within this loop; the saturation claim only refers to the dose range where append-mode dynamics are intact.

#### 5.1.3 Buffer-size sensitivity: does the saturation depend on the memory policy?

*[Sensitivity-experiment results — to be inserted when run completes.]*
...
![Fig 5. **Cross-loop overwrite-versus-insert switching (round-32, F3).** Matched adversarial doses 80 and 200 across O1 append, O2 oscillatory replace, and O3 absorbing replace; n=50 trajectories per cell. Solid bars show state-reset overwrite; hatched bars show insert; error bars are Wilson 95% CIs. **Two takeaways.** (a) The overwrite-insert gap is small in O1 append (14-34 pp) and large in O2/O3 replace (60-80 pp), consistent with the state-write tautology being confined to replace-mode update rules. (b) Insert-mode rates follow \(O1 \ge O2 \ge O3\) at both doses, showing that insert switching is regime-conditional rather than a regime-invariant model-behavior constant. Source: `data/aggregated/overwrite_vs_insert_cross_regime/cross_regime.png`.[^Fig5]](data/aggregated/overwrite_vs_insert_cross_regime/cross_regime.png)
...
![Fig 10. **Switching under alternative basin granularities.** Perturbation switching recomputed under K-means $k=12$, K-means $k=4$, and HDBSCAN. The O1 adversarial-versus-OOD contrast is robust across granularities, while D1 is more granularity-sensitive. Source: `data/aggregated/multi_granularity_switching.png`.[^Fig10]](data/aggregated/multi_granularity_switching.png)
...
![Fig 12. **Embedding-model ablation.** Diagnostics recomputed under `text-embedding-3-small`, `text-embedding-3-large`, and `all-mpnet-base-v2`. Basin predictability and coarse recurrence ordering are more stable than sharpness dimension. Source: `data/aggregated/embedding_ablation/comparison.png`.[^Fig12]](data/aggregated/embedding_ablation/comparison.png)
...
![Fig 13. **V* parameter-grid sensitivity.** Sensitivity of empirical potential-barrier summaries across KDE bandwidth, grid resolution, and basin-count settings. The ordinal pattern is more stable than the absolute $V^\star$ values, so density landscapes remain descriptive rather than calibrated. Source: `data/aggregated/v_star_sensitivity.png`.[^Fig13]](data/aggregated/v_star_sensitivity.png)
...
![Fig 14. **Regime clustering in diagnostic space.** Scatter view of regime diagnostic vectors used in the unsupervised five-regime check. Bulk geometry separates replace-mode regimes from append/dialog regimes but does not by itself recover the full five-way taxonomy. Source: `data/aggregated/regime_cluster_analysis/cluster_scatter.png`.[^Fig14]](data/aggregated/regime_cluster_analysis/cluster_scatter.png)
...
![Fig 15. **Regime-clustering dendrogram.** Hierarchical clustering of regime-level diagnostic summaries. The dendrogram reinforces that the five-regime taxonomy is not obtained from bulk diagnostics alone and requires perturbation endpoints for separation. Source: `data/aggregated/regime_cluster_analysis/cluster_dendrogram.png`.[^Fig15]](data/aggregated/regime_cluster_analysis/cluster_dendrogram.png)
...
![Fig 16. **Cross-experiment temperature sensitivity.** Temperature-sensitivity summary across reduced-scope cells. These results remain explicitly secondary because absolute O1 values are scope-confounded relative to the publication-scale anchor. Source: `data/aggregated/t_sensitivity_cross_regime/cross_t_sensitivity.png`.[^Fig16]](data/aggregated/t_sensitivity_cross_regime/cross_t_sensitivity.png)
...
| O1 neutral | switching @ dose 200 | 0.24 | Wilson [0.13, 0.38] | §5.4 / Fig 8 | [OK] sparse pilot |
| O1 neutral | switching @ dose 400 | 0.18 | Wilson [0.08, 0.32] | §5.4 / Fig 8 | [OK] sparse pilot |
| O1 lorem | switching @ dose 200 | 0.18 | Wilson [0.08, 0.32] | §5.4 / Fig 8 | [OK] sparse pilot |
...
![ED Fig 6. **Combined t-SNE flow fields.** t-SNE-space flow-field visualization used for qualitative comparison with the PCA-2 flow summaries. Source: `data/aggregated/dynamics_plots/E_tsne_flow_fields_rolling_k3.png`.[^EDFig5]](data/aggregated/dynamics_plots/E_tsne_flow_fields_rolling_k3.png)
...
![ED Fig 11. **Representative O1 perturbation potential landscapes.** PCA-2 density landscapes for the O1 perturbation pilot under control, neutral, lorem, and adversarial conditions. The landscapes are descriptive geometry, not calibrated token barriers. Source: `data/exp_perturb_O1_pilot/reports/perturbation/bulk_landscape_pca.png`.[^EDFig10]](data/exp_perturb_O1_pilot/reports/perturbation/bulk_landscape_pca.png)
...
![ED Fig 15. **Representative O1 RG dendrogram.** Ward-merge cloud-expansion dendrogram for the O1 perturbation pilot. The figure supplements the geometric-barrier table with an independent view of condition-wise cloud expansion. Source: `data/exp_perturb_O1_pilot/reports/perturbation/rg_dendrogram_pca.png`.[^EDFig14]](data/exp_perturb_O1_pilot/reports/perturbation/rg_dendrogram_pca.png)
