...
appear to switch near 100% do so because the default intervention is
*state-reset overwrite* — the perturbation literally becomes the next
state, which is tautological at the access step; insert-mode probes
that decouple model exposure from state-write reduce switching to
12-32%.
...
This summary uses a few paper-internal labels. Translated up front so
the rest reads cleanly:
...
- **Paired control.** The same prompt run twice with only random
  sampling differing. Used to ask "is the difference we observe
  bigger than what randomness alone produces?"
- **Injection (perturbation, dose).** A chunk of text we splice into
  the loop at a chosen step. Its size is measured in tokens (LLM
  word-pieces); we vary the dose from 5 to 400 tokens.
- **Append-mode vs replace-mode.** Two ways the loop maintains its
  context. *Append-mode* keeps everything and adds each new turn at
  the end (like a transcript). *Replace-mode* discards prior context
  and rewrites it from the latest output (like overwriting a draft).
  ...
  next state, so high switching there is a property of the update
  rule, not the model. The paper's *insert-mode* counterfactual
  (§5.2) is what isolates model behavior from this state-write
  artifact.
- **In-distribution adversarial text.** Injected text that is
  hand-crafted to look like it naturally belongs in the conversation,
  rather than obviously off-topic gibberish.
...
- **ED50.** Half-effect dose, borrowed from pharmacology: the dose
  at which the effect crosses 50%. $\mathrm{ED50}_{\mathrm{raw}}$
  is the dose at which raw switching crosses 50%;
...
- **Embedding space.** The numeric vector representation of text
  ...
  in similar regions across runs; it is *not* a factuality claim.
...
**Claim 4: replace-mode apparent fragility is largely update-rule overwrite, not necessarily weak attractor structure (§5.2).** Replace-mode paraphrase and summarize-and-negate loops show near-saturated raw switching across tested doses, but the default intervention is *state-reset overwrite*: the perturbation replaces \(Y_t\), and replace-mode update then sets \(X_{t+1}=\operatorname{clip}(\text{perturbation\_text})\). Switching at the access step is therefore tautological — the next state simply *is* the perturbation. Insert-mode probes, which expose the model to the perturbation but preserve the model's own output as the state, reduce switching to 12 to 32%. This separates state-write access from model-mediated redirection and makes memory policy a safety-relevant design choice.
...
- **Perturbation switching signature:** O1 selective sensitivity passes, with \(S_{\mathrm{adv}}(200)=0.620\) in the dense rerun and a ratio to neutral or lorem switching of approximately 2.8. Replace-mode O2/O3 capitulation passes by point estimate but is **tautological at the state-access step** (replace-mode intervention sets \(X_{t+1}=\operatorname{clip}(\text{perturbation\_text})\)); only the post-injection convergence under \(f\) is empirical, and the insert-mode counterfactual (§5.2) is the appropriate model-behavior measurement.
...
**Tautology warning before the headline numbers.** In replace-mode, the context-update rule is \(X_{t+1}=\operatorname{clip}(Y_t)\): only the latest output becomes the next state. The original ("overwrite") perturbation protocol overrides \(Y_t\) with the injected text, which under replace-mode update yields \(X_{t+1}=\operatorname{clip}(\text{perturbation\_text})\). The injection at the access step is therefore *tautological at the state-write level*: the next state simply *is* the perturbation, and any post-injection trajectory is the convergence of the operator \(f\) starting from the perturbation as a fresh initial condition. This is **state-reset overwrite**, not a measurement of model resistance to redirection. We report the naive numbers below to expose the trap, then correct via the insert-mode counterfactual.
...
Read alone, these numbers suggest that replace-mode regimes have almost zero injected-token barriers. The overwrite-versus-insert probe shows that this is mostly a memory-policy effect; the high overwrite rates are the access tautology in action, not evidence of weak attractor structure. We re-ran O2 and O3 with the same adversarial doses under two intervention modes that decouple state-write from model exposure:
...
#### 5.2.1 Cross-loop insert validation

The §5.2 overwrite-vs-insert decomposition was run only in replace-mode (O2, O3). To test whether the insert-mode estimand is interpretable as a regime-independent measurement of model behavior, we ran the same protocol in append-mode O1: 5 families × 5 ICs × 2 runs × 5 conditions = 250 trajectories, gpt-4o-mini, matched to the O2/O3 design (`configs/perturbation/O1_overwrite_vs_insert.yaml`). Final-cluster switching against the paired control:
...
Two observations relative to the O2/O3 numbers above.

**Observation 1: the overwrite-vs-insert gap is much smaller under append-mode than under replace-mode.**
...
This is consistent with the §3.1 access bound and the §5.2 framing. Append-mode has no state-write tautology: the default ("overwrite") protocol appends the perturbation tokens to the existing transcript rather than promoting them to the entire next state, so the overwrite rate already reflects model-mediated effects on subsequent generations rather than a state-access artifact. The remaining 14-34 pp append-mode gap is plausibly attributable to the difference between perturbation tokens persisting in the appended history (overwrite) versus disappearing after one API call (insert), not to a state-replacement mechanism. The 60-80 pp replace-mode gap is dominated by the access tautology.

**Observation 2: insert-mode switching is regime-conditional, not regime-invariant.**

At matched dose, insert-mode rates differ across loop modes:
...
The Wilson CIs of O1 (append) and O3 (absorbing replace) do not overlap at either tested dose, and the ordering O1 ≥ O2 ≥ O3 is consistent at both doses. This pattern weakens any reading of the §5.2 12-32% replace-mode insert numbers as a measurement of "pure model behavior independent of update rule." A more accurate framing: under insert protocol, the perturbation imprints on \(Y_{t_{\mathrm{inj}}}\) for one generation; the loop's update rule then determines how much of that imprint propagates. Append-mode preserves \(Y_{t_{\mathrm{inj}}}\) in a growing transcript, so any cluster-relevant signal in \(Y_{t_{\mathrm{inj}}}\) carries forward through subsequent appended turns. Replace-mode iterates \(f\) on its own output, and the operator's natural attractor structure (oscillatory for O2, absorbing for O3) erases the imprint within a few post-injection steps; absorbing dynamics erase faster, consistent with the observed O3 < O2 < O1 ordering.

**Caveat on scope.** §5.2.1 is a 50-trajectory-per-condition cross-validation cell, not a publication-scale rerun; CIs are wide and small per-cell differences (for example O1 versus O2 at dose 80, where CIs partly overlap) are not statistically resolved. The purpose of §5.2.1 is methodological — to validate the insert-mode protocol across loop modes — rather than to refine the dose-response curves of §5.6. The headline append-mode results (\(\mathrm{ED50}_{\mathrm{raw}}\approx 40\) tokens, plateau ≈ 0.67, persistent escape not reached up to 400 tokens) are based on the publication-scale dense rerun (`exp_perturb_O1_ed50_dense`) and are unchanged.

The cross-loop check produces a useful refinement of the §5.2 framing rather than a contradiction: the 12-32% replace-mode insert numbers are correct for replace-mode but should not be read as a regime-invariant model-behavior measurement. F1 in §8.6 (visibility × hard-write factorial across all loop modes) is the principled way to fully disentangle the remaining model-versus-update-rule interaction; F3's 250-trajectory check is a near-term sanity check that motivates F1 as future work.
...
![Fig 5. **Leakage-aware basin predictability.** Group-aware basin-predictability with prompt families held out across folds. O1 remains the strongest leakage-free predictability result, while O2, O3, and D1 drop substantially under family-held-out validation. Source: `data/aggregated/group_aware_basin_pred.png`.[^Fig5]](data/aggregated/group_aware_basin_pred.png)
...
![Fig 6. **Cross-experiment dynamics map.** Regime-level map in late-window $\lambda_1$ versus sharpness-dimension space, showing broad separation of replace, append, and dialog regimes. The plot is diagnostic rather than endpoint-defining. Source: `data/aggregated/dynamics_plots/regime_map_rolling_k3.png`.[^Fig6]](data/aggregated/dynamics_plots/regime_map_rolling_k3.png)
...
![ED Fig 6. **Original stratified basin-predictability curves.** Stratified-CV basin-predictability curves are retained for audit but should be interpreted alongside the leakage-aware GroupKFold results in main-text Fig 5. Source: `data/aggregated/basin_predictability_cross/cross_basin_predictability.png`.[^EDFig6]](data/aggregated/basin_predictability_cross/cross_basin_predictability.png)
