...
[^Fig1]: This orientation plot shows that more perturbation does not have one universal meaning in recursive language-model loops. In the dialog-style recursive loop given neutral, topic-irrelevant filler, the system is already highly disrupted at the smallest tested doses: roughly six in ten trajectories switch basins at only 5 tokens, 62 percent, and the curve stays high, around 68 to 78 percent, as dose increases. That looks fragile, but it is not a real dose response, because the effect is essentially saturated everywhere: even a tiny amount of off-topic text is enough to knock the loop away. The more revealing comparison is within the same append-mode continuation architecture. With neutral, off-topic filler, switching stays near the sampling-noise floor, about a quarter of trajectories, 22 to 26 percent, even when the injected text grows from 20 to 400 tokens. With in-distribution perturbations, meaning text drawn from another trajectory of the same kind of loop, the same append-mode system behaves very differently: switching rises from about a quarter, 26 percent, to just over half, 54 percent, by 200 tokens, before slightly dropping to 48 percent at 400 tokens. Thus the diagnostic signal is not raw fragility, but content-matched dose sensitivity. This is why the rest of the paper separates mere switching from durable basin escape: some curves show disruption, while others show a structured response to perturbation strength.
