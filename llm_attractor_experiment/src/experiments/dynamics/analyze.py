...
def analyze_experiment(cfg: Config, write_per_experiment_csv: bool = True) -> pd.DataFrame:
    ...
    for obs_name in cfg.observables:
        ...
        for (regime, family, ic), runs_by_step in ensembles.items():
            ...
            # SD = Tuci-style fractional dimension over the Lyapunov
            # spectrum (ARTICLE.md §4.5.6). Takes Lyapunov exponents
            # directly, not singular values — see sharpness_dim.py
            # docstring for why participation ratio is degenerate at N=3.
