...
\renewcommand{\shorttitle}{Endogenous attractor regimes in recursive LLM loops}

% `\\{}` (with the empty group) prevents \\ from parsing the next
% `[...]` as its optional vertical-space argument.
...
\hypersetup{
  pdftitle={Endogenous attractor regimes in recursive large-language-model loops},
...
asked the next question: \textbf{what does it cost to push a trajectory out
of an attractor?}
...
framework, a \textbf{nudge} is the operator that maps the current context
...
the \textbf{barrier height} of an attractor as the token-cost of injected
text required for 50\% trajectory switching — an interpretable
operational unit for comparing recursive regimes.

Across 37 experiments on \texttt{gpt-4o-mini}, with 50–1350 trajectories per
configuration embedded using \texttt{text-embedding-3-small}, we identify
...
We then compare these \textbf{behavioral} barriers to \textbf{geometric} barrier
...
their \textbf{resistance to nudging}.

The full pipeline regenerates from raw trajectories
(\texttt{steps.jsonl}, LFS-tracked) using a documented \texttt{embed → analyze →
...
claims are cell-verified against the published CSVs (\texttt{RESULTS.md}).
...
Tuci et al. (2026, \citep{arxiv260419740}) study SGD-optimization dynamics
...
\subsubsection{Replace-mode access bound (Lemma 1 + Corollaries)}
...
\begin{lemma}[replace-mode access bound]
...
\begin{proof}
 Fix a trajectory with $X_{t_{\mathrm{inj}}} \in B_1$. For
...
that $\Pr(X_T \in B_2) \ge 1 - (1 - q_0 r_0)^m$.
\end{proof}


\begin{corollary}[replace-mode barrier bound]
...
\begin{proof}
 By Lemma 1, after $m$ replace steps the terminal switching
...
switching is bounded above by $\kappa \, m^\star$.
\end{proof}
...
\begin{proof}
 If $q_0 r_0 \ge \tfrac{1}{2}$, then a single replace step
already yields terminal switching probability at least $1/2$. The
barrier is therefore at most the expected cost of one generation.
\end{proof}
...
\subsubsection{Tokens vs nats: a model-agnostic reading of barrier height}

A natural objection to reporting barrier heights in tokens: \textit{tokens
...
\textbf{The model-agnostic quantity is barrier height in \textit{nats}.} For a
...
\textbf{(a) Out-of-distribution perturbations carry low \textit{basin-relevant}
...
\textbf{Practical consequence.} A claim of the form \textit{"O1 has a 150-token
barrier"} is a tokenizer-specific quantity; the underlying
\textit{information-theoretic} claim is \textit{"O1 has a barrier of $150
...
weight = V at endpoint). The maximum V along the path is the \textbf{barrier
height V*(i, j)}.
...
\caption{Cross-experiment regime map (joint t-SNE on rolling\_k3 observable). Each point is a single trajectory's late-window centroid; each color a regime / experiment. The four (plus one) regimes occupy visibly distinct regions of the embedding space — the taxonomy is not an artifact of any specific projection.}
\end{figure}
...
\subsection{Phase 1 — the four-regime taxonomy at small N}
...
the V\_max ≈ 8.0 ceiling appears when a geodesic crosses a region of
near-zero density.)
...
\begin{itemize}
  ...
  traverse low-density plateaus where ρ ≈ ε (V near the V\_max
...
\caption{Embedding-space invariance ablation. Three panels: recurrence rate (left), sharpness\_dim\_late (middle), and basin\_pred\_acc(k=10) (right) per regime, grouped bars per embedder. Recurrence preserves its bimodal high/low structure (O2/O3 high in all embedders; O1/D1/D2 low). Basin predictability preserves the full regime ordering (Spearman ρ ≥ 0.80 vs baseline). Sharpness dim shifts substantially across embedders — the regime ordering is not preserved (ρ = 0.00 vs mpnet). Source: \texttt{data/aggregated/embedding\_ablation/comparison.png}.}
\end{figure}
...
\section{Coverage of original specification}
...
\subsection{Implementation phases (req1.txt 12-phase plan)}
...
7 & Recurrence with \texttt{‖z\_t − z\_s‖ < ε}, \texttt{|t − s| > τ} & \texttt{src/analysis/recurrence.py} \\
...
10 & Three baselines (no\_feedback, time\_shuffled, independent\_regeneration) & \texttt{src/core/baselines.py} \\
11 & Robustness across observables, spaces, seeds & \texttt{src/analysis/robustness.py} + cross-observable comparison in reports \\
12 & Markdown report with not\_supported / weak / moderate / strong classification & \texttt{src/reports/summary.py} (\texttt{classify\_two\_axis}, \texttt{classify\_three\_axis}) \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{OpenAI API surfaces (req2.txt)}
...
\subsection{H1a/H1b two-axis split (reg4.txt)}
...
\subsection{Methodological evolutions}

The implementation deviates from the original brief in three places.
We document each to be honest about the divergence:

\subsubsection{Perturbation: initial-condition → mid-trajectory injection}

req1.txt Phase 9 specifies basin score via \textit{perturbed initial
...
\subsubsection{Temperature sweep: 4 levels instead of warm/cool pair}
...
\subsubsection{Observable: rolling\_k3 kept, rolling\_k5 not added}
...
\subsection{Reports / classification format (req1.txt §12)}
...
\subsection{What the original brief did NOT call for but we added}
...
\citet{arxiv260419740} framework for finite-time Lyapunov spectra of
...
% \bibliographystyle{unsrtnat}  % inlined for arXiv submission
% \bibliography{refs}  % inlined for arXiv submission
...
\bibitem[Tacheny(2025)]{arxiv251210350}
N~Tacheny.
\newblock Geometric dynamics of agentic loops in large language models, 2025.

\bibitem[Du and Tanaka-Ishii(2025)]{arxiv251021258}
X.~Du and K~Tanaka-Ishii.
\newblock Correlation dimension of auto-regressive large language models, 2025.

\bibitem[Berg et~al.(2025)Berg, de~Lucena, and Rosenblatt]{arxiv251024797}
C.~Berg, D.~de~Lucena, and J~Rosenblatt.
\newblock Large language models report subjective experience under
  self-referential processing, 2025.

\bibitem[Tuci et~al.(2026)Tuci, Korkmaz, Şimşekli, and
  Birdal]{arxiv260419740}
M.~Tuci, C.~Korkmaz, U.~Şimşekli, and T~Birdal.
\newblock Generalization at the edge of stability, 2026.
