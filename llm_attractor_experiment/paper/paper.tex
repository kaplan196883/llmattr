...
\caption{\textbf{Basin-predictability across regimes.} Top-1 accuracy for predicting each trajectory's late-window K-means cluster from its embedding at step $k$, using publication-scale runs where available. Source: \texttt{data/aggregated/basin\_predictability\_cross/cross\_basin\_predictability.png}.}
\end{figure}
...
\caption{\textbf{Perturbation switching by regime.} Grouped bar chart of final-step switching rate, defined as final K-means cluster disagreement with the paired control trajectory. Source: \texttt{data/aggregated/perturbation\_cross\_regime/cross\_switching\_rates.png}.}
\end{figure}
...
\caption{\textbf{Switching versus injection time.} Switching rate for injections at steps 5, 15, and 25 of a 30-step trajectory, with $n=50$ per cell and 95\% Wilson confidence intervals. Source: \texttt{data/aggregated/perturbation\_basin\_hardening/basin\_hardening.png}.}
\end{figure}
...
\caption{Figure H. \textbf{Switching rates under alternative cluster granularities.} Perturbation switching rates for D1, O1, O2, and O3 recomputed with K-means $k=12$, K-means $k=4$, and HDBSCAN. Source: \texttt{data/aggregated/multi\_granularity\_switching.png}.}
\end{figure}
...
\caption{Figure I. \textbf{Per-family O1 adversarial dose-response.} Lines show switching rate versus adversarial dose for each prompt family in the sparse O1 dose experiment, with $n=10$ trajectories per family-dose cell. Source: \texttt{data/aggregated/per\_family\_ed50.png}.}
\end{figure}
...
\caption{\textbf{Embedding-model ablation.} Bar plots compare three diagnostics across regimes after re-embedding subsamples with \texttt{text-embedding-3-small}, \texttt{text-embedding-3-large}, and \texttt{all-mpnet-base-v2}. Source: \texttt{data/aggregated/embedding\_ablation/comparison.png}.}
\end{figure}
...
\caption{\textbf{Representative PCA-2 density landscapes for the O1 perturbation pilot.} Panels show $V(x) = -\log \rho(x)$ for control, neutral, lorem, and adversarial conditions. Source: \texttt{data/exp\_perturb\_O1\_pilot/reports/perturbation/bulk\_landscape\_pca.png}.}
\end{figure}
