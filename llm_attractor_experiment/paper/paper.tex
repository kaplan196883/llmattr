...
\citep{arxiv251210350,arxiv251021258,arxiv251024797}, but has not
asked the next question: \textbf{how much injected text does it take to
...
\textbf{Headline finding (O1, append-mode continuation, $n=200$/cell × 8
doses × 1,800 trajectories, §5.6.1):} in-distribution adversarial
injections produce a reproducible \textbf{raw-switching ED50 ≈ 40 tokens}
(4PL pooled = 36; GLMM = 41; family-cluster-bootstrap median = 52,
95\% CI [8.5, 242]) — but the strict \textbf{persistent-escape barrier is
not reached in the tested 5–400 token range}. At 400 tokens, only
\textbf{16\% (k=12) / 10\% (k=4) / 39.5\% (HDBSCAN)} of trajectories are
kicked into a new cluster \textit{and} remain there at the terminal step.
Raw switching saturates at a \textbf{plateau near 0.67} rather than 1.0,
and a \textbf{control-vs-control stochastic divergence floor of 0.35}
(n=600 paired comparisons) consumes most of the apparent effect at
low doses; the \textit{net} adversarial effect saturates at +32 pp above
...
\textbf{Replace-mode tautology probe (O2, §5.10.11).} A direct test
...
under state-overwrite (the existing protocol) but only \textbf{18–32\%
...
\textit{no formal injected-token barrier} — the bound is on a separate
...
Across 37 experiments on \texttt{gpt-4o-mini}, we identify \textbf{four diagnostic
...
dialog. We additionally report \textbf{preliminary evidence} for a fifth
...
(\texttt{steps.jsonl}, LFS-tracked). All code, configs, raw data, and
reports are publicly available (<https://github.com/kaplan196883/llmattr>).

---
\end{abstract}

\section{Raw switching, stochastic floors, and rare persistent escape across append, replace, and dialog nudges}
...
\begin{lemma}[replace-mode hitting bound]
 Let $(X_t)_{t\ge 0}$ be a
...
\begin{proof}
 Hitting bound: induction on $A_k = \{\sigma_2 > t_{\mathrm{inj}} + k\}$ using assumption (1) gives $\Pr(A_m) \le (1-q_0)^m$. Terminal bound: decompose over the first hitting time and use the Markov property with assumption (2). Generation budget: tower property with assumption (3). Full proof in §13.1 (Supplementary appendix).
\end{proof}


\begin{corollary}[replace-mode generation-budget bound]
 For the fixed
...
\begin{proof}
 Take $m = m_{1/2}$ in Lemma 1; the bounds follow directly. Full proof in §13.1.
\end{proof}
...
\begin{corollary}[one-generation special case]
 If
...
\begin{proof}
 Take $m = 1$ in Lemma 1.
\end{proof}
 (Full step-by-step proofs of Corollaries 1 and 2 in §13.1.)
...
\begin{conjecture}[append-mode accumulation barrier]
 Let
...
\begin{verbatim}
   ...
   |      | (N_trajxT strings|                       | text-embedding-3 |       |
   ...
   |                                                                            |
   |   significance gate:  metric >= baseline + 2sigma  AND  Cohen's d >= 0.5         |
   |                                                                            |
   |                                  |                                         |
   |                                  v                                         |
   ...
   |     | H1a strong + sharpness v => absorbing                  (O3)       |   |
   ...
   |  | dwell_dist          |      | -> I: divergence grad.v |                      |
   |  | step_parity         |      |      (RdBu_r)       |                      |
   |  +---------------------+      +---------------------+                      |
   |                                                                            |
   |  +---------------------------------------------------------------------+   |
   ...
   |                                                                            |
   |   +--------------------------------------------------------------------+   |
   |   | aggregate_perturbation_cross_regime  -> 4x5 switching grouped bar   |   |
...
\begin{asciiblock}
\begin{verbatim}
   raw/steps.jsonl                   ~54,000 rows  (1350 traj x 40 steps)
        |
        v  build_all_for_run x 4 observables
   ~216,000 strings per experiment
        |
        v  embed_texts (batched 128, retry+backoff)
   embeddings/<obs>/embeddings.npy   (54000, 1536)  float32, L2-normalized
   embeddings/<obs>/metadata.parquet (54000 rows)
        |
        v  PCA(n=10).fit(joint) + KMeans(k=12)
   PCA-10:    (54000, 10)
   clusters:  (54000,)  in {0..11}
        |
        v  per-trajectory metrics
   recurrence.csv:  (1350 trajectories x N_metrics columns)
   dwell.csv, basin.csv, basin_entry.csv, exit_return.csv,
   late_recurrence.csv, periodicity.csv, dispersion.csv
        |
        v  per-(family, ic) ensemble dynamics
   lyapunov_spectrum.csv:  (15 family-ic pairs x T steps x top-k lambda)
   sharpness_dim.csv:      (15 family-ic pairs x T steps)
        |
        v  bootstrap + permutation + Cohen's d
   bootstrap_summary.csv, effect_sizes.csv
        |
        v  three-axis classifier
   ThreeAxisDecision: {h1a, h1b, h1c} in {not_supported, weak, moderate, strong}
        |
        v  reports/plots + reports/perturbation
...
\midrule
\textbf{Operational attractor score C1–C4} & Count of the four attractor criteria passed: late-window basin persistence, recurrence/dwell above null, embedder robustness, and contraction / re-entry / collapse. & Publication-scale O1, O2, O3, D1 on canonical observables; D2 exploratory status checked separately. Summary table in §3.1.1.5. & \textbf{Strong attractor:} 4/4 criteria PASS. \textbf{Attractor-like:} ≥3/4 PASS. \textbf{Not attractor:} <3/4 PASS. Missing publication-scale measurements count as FAIL unless structurally inapplicable. & §3.1.1.5; metric components in §§4.5.1–4.5.7 \\
\textbf{Leakage-free basin predictability acc\_group(k=10)} & GroupKFold-by-prompt-family accuracy of predicting the late-window K-means basin from the PCA-10 state at step k=10. & Publication-scale O1/O2/O3/D1, \texttt{context\_tail}, K-means k=12, \texttt{data/aggregated/group\_aware\_basin\_pred.csv}. & To claim \textbf{cross-family basin predictability}: acc\_group(k=10) ≥ \textbf{0.70}. To claim the original stratified number is \textbf{leakage-free}: Δ = acc\_stratified − acc\_group < \textbf{0.10}. & §4.5.10; group-aware stress test §5.10.5 \\
\textbf{Perturbation switching signature} & Final-step switching rate: fraction of perturbed trajectories whose final K-means cluster differs from the paired control trajectory. & O1 dose-response at matched 200-token dose; O2/O3/D1 perturbation pilots; \texttt{data/aggregated/perturbation\_cross\_regime/} and \texttt{data/aggregated/perturbation\_dose\_response/}. & \textbf{O1 selective sensitivity:} S\_adv(200) ≥ \textbf{0.50} and S\_adv(200) / max(S\_neutral(200), S\_lorem(200)) ≥ \textbf{2.0}, with max OOD switching ≤ \textbf{0.30}. \textbf{Replace-mode capitulation:} min non-control switching across O2/O3 neutral/lorem/adversarial ≥ \textbf{0.85}. & §4.5.11; behavioral results §§5.5–5.6 \\
\textbf{Behavioral ED50 token barrier} & The perturbation dose τ at which a 4-parameter logistic fit to the O1 adversarial dose-response reaches 50\% switching, with prompt-family-cluster bootstrap uncertainty. & O1 adversarial dose sweep τ ∈ {20, 80, 200, 400}, n=50/cell; \texttt{fit\_ed50\_hierarchical.py}; reported in §5.0bis / §5.6. & To claim a \textbf{localized token barrier}: ED50 point estimate finite and the 95\% family-cluster bootstrap CI lies wholly inside the probed interval \textbf{[20, 400] tokens}. If the point estimate is inside but the CI crosses the interval boundary, report only "finite but unlocalized / in flight." & Barrier definition §3.1.1; dose protocol §4.5.11; ED50 analysis §5.6 \\
\textbf{Persistent basin-escape rate} & Fraction of trajectories that visibly change cluster at injection AND remain in that post-injection cluster at the terminal step. & O1 adversarial sparse-dose sweep using \texttt{joint\_pca10\_clusters.csv}; summary in \texttt{data/aggregated/persistence\_summary.csv}. & To interpret switching as \textbf{persistent basin escape} rather than final-step divergence: persistent escape rate ≥ \textbf{0.50} at the claimed barrier dose. If <0.50, switching may still be reported, but not as clean basin escape. & Switching definition §4.5.11; persistence reanalysis §5.10.9 \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
\textbf{O1} contractive & append & continue & 0.77 → 0.85 & low & 1.70 & 54\% (sparse) / 62\% (dense, n=200) & ED50 ≈ 40 tok (4PL/GLMM/bootstrap), plateau ~67\%, natural floor ~35\% & degrades smoothly \\
...
\textbf{reported} dose-response barrier-height measurement for an LLM
...
D1 stylistic dialog & 450 & 0.604 & 0.336 & \textbf{+0.269} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
  ...
  \item \textbf{D1 (Δ = +0.27)} behaves similarly — the stylistic basin is
...
contribution is \textbf{operator-independent}.
...
T5 & D1 neutral 0.76 & D1 neutral 0.80, lorem 0.94 & direction holds; D1 stylistic susceptibility is \textbf{larger} on nano \\
...
\begin{enumerate}
  \item \textbf{The smaller model's D1 stylistic basins are easier to flip.}
...
\begin{tabularx}{\textwidth}{lYYY}
\toprule
if you want… & choose… & what we actually measured & what is \textit{not} directly tested \\
\midrule
...
\textbf{fast lock-in to a topic} (don't care which one) & replace-mode (O2 paraphrase or O3 summarize+negate) & 90–98\% switching with overwrite-mode injection (§5.10.11), but \textbf{only 12–32\% with context-insert injection at the same doses} — the 60–80 pp gap is the operator-overwrite contribution. The pattern replicates across both replace-mode regimes (O2 and O3), confirming the contribution is operator-independent. Per Lemma 1 / §3.1.2, replace mode has no formal injected-token barrier; the generation-budget result is the operator's contribution, not a discovered low barrier & whether insert-mode switching rises further at larger doses \\
\textbf{stylistic stability across resets} & dialog framework (D1) & basin-predictability acc(k=10) is in [0.57, 0.61] across T ∈ {0.3, 0.6, 0.9, 1.2} (§5.4); 4-of-5 attractor criteria PASS (§3.1.1.5) & persona stability under adversarial roleplay or multi-turn manipulation (no such experiments run) \\
\textbf{content gravity that resists topic-switching} & structured drill-down dialog (D2) & 64\% adversarial switching rate at one dose, n=25 (§5.8) & publication-scale validation (D2 fails the operational attractor criteria in §3.1.1.5 due to underpowered measurements) \\
\textbf{collapse} (degenerate output) & replace-mode summarize+negate (O3) & convergence within ~10 steps; sharpness-dim ≈ 1.3–1.5 (lowest measured; §5.0); trivially low effective rank & none — the empirical signature is direct \\
\bottomrule
\end{tabularx}
\end{table}
...
attractor status under the \textit{non}-stress-tested C1, which is the
...
\begin{itemize}
  ...
  \item §3.1.3 effective-context-share formulation, §3.1.3 geometric
