...
\citet{arxiv251021258} quantifies degeneration as a collapse from a
higher-dimensional trajectory into a low-dimensional attractor.
\citet{arxiv251024797} reports that recursive self-referential dialogues
...
\citet{arxiv251021258} (\textit{Correlation Dimension of Auto-Regressive Large
...
geometric/behavioral triangulation & n/a & \textbf{mean V* across 6 inter-basin geodesics agrees with perturbation-derived dose thresholds} (§5.10) \\
...
"nudge" interchangeably in places (\texttt{paper/openai\_review.md} Writing
...
\subsubsection*{3.1.1bis Three operational endpoints for "barrier height"}
...
\begin{itemize}
  ...
  (sharpness dim, in particular, is \textit{not} embedder-invariant per §5.13).
...
\midrule
\textbf{O1 contractive} & PASS: $A^{\mathrm{final}}=0.85$ (§5.3) & PASS by dwell / basin-null gate; raw recurrence low, $R=0.29$ (§5.11) & PASS: recurrence remains low across embedders, $0.289/0.304/0.096$ (§5.13) & PASS: $\lambda_1^{\mathrm{late}}\approx 0.008\le 0.015$; $SD=1.70$ (§5.0, §5.12) & adversarial switch $54\%$; OOD floor $18$–$24\%$ (§5.5–§5.6) & \textbf{attractor (strong)} \\
\textbf{O2 oscillatory} & PASS: $A^{\mathrm{final}}=0.91$ (§5.3) & PASS: high recurrence / period-2 structure, $R=0.875$ (§5.11, §5.13) & PASS: high recurrence under all embedders, $0.875/0.711/0.783$ (§5.13) & PASS: period-2 re-entry; $SD=1.39$ (§5.0) & adversarial switch $94\%$ (§5.5) & \textbf{attractor (strong)} \\
\textbf{O3 absorbing} & PASS: $A^{\mathrm{final}}=0.93$ (§5.3) & PASS: trivial high recurrence, $R=0.924$ (§5.11, §5.13) & PASS: high recurrence under all embedders, $0.924/0.850/0.862$ (§5.13) & PASS: absorbing-collapse gate, $R\ge 0.90$, $SD=1.45\le 1.50$ (§5.0, §5.13) & adversarial switch $96\%$ (§5.5) & \textbf{attractor (strong)} \\
\textbf{D1 dialogue-state multi-basin} & PASS stratified: $A^{\mathrm{final}}=0.77$ (§5.3); \textbf{FAIL group-aware: 0.34} (§5.10.5) & PASS by dwell / basin-null gate; raw recurrence low, $R=0.21$ (§5.11) & PASS: recurrence remains low across embedders, $0.210/0.337/0.226$ (§5.13) & PASS: $\lambda_1^{\mathrm{late}}\approx 0.011\le 0.015$; $SD=1.89$ (§5.0, §5.12) & adversarial switch $60\%$; neutral $76\%$ (§5.5) & \textbf{attractor-like dialog regime} (formal C1–C4 PASS; group-aware C1 fails) \\
\textbf{D2 drill-down} & NT / FAIL: no publication-scale final basin-predictability cell; exploratory $A(k=5)=0.20$ with $n=25$ (§5.0) & NT / FAIL: no null-calibrated recurrence / dwell test at publication scale (§5.0, §7.7) & PARTIAL only: recurrence low across embedders, $0.296/0.176/0.073$, but small-$N$ (§5.13) & NT / FAIL: Lyapunov / exit-return not measured; $SD$ unavailable (§5.0) & adversarial switch $64\%$ (§5.8) & \textbf{not attractor} under this definition; exploratory regime candidate \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
  ...
  intervention; see §5.6.1 / §13.4 / \texttt{paper/ed50\_rerun\_plan.md}).
...
\subsection*{5.0bis Unified primary-results table}
...
O1 & basin predictability acc(k=10), group-aware & \textbf{0.73} & family-cluster GroupKFold & §5.10.5 (this revision) & [OK] leakage-free \\
...
O1 adv & $\mathrm{ED50}_{\mathrm{raw}}$ (dense) & \textbf{36–52 tok} & 4PL=36; GLMM=41; family-cluster bootstrap median=52, 95\% CI [8.5, 242] & §5.6.1 / §3.1.1bis & [OK] established \\
...
O1 adv & $\mathrm{ED50}_{\mathrm{net}}$ (dense, raw − floor) & \textbf{not reached} & max net effect = +0.323 at dose 400 (50 pp threshold) & §3.1.1bis & not reached in tested range \\
O1 adv & $\mathrm{ED50}_{\mathrm{persist}}$ (dense, kicked-AND-persisted) & \textbf{undefined} & max persistent escape = 16\% at dose 400 & §5.10.9 (dense) / §3.1.1bis & not reached in tested range \\
...
O2 & basin predictability acc(k=10), group-aware & \textbf{0.60} & family-cluster GroupKFold & §5.10.5 & [OK] leakage-free \\
...
O3 & basin predictability acc(k=10), group-aware & \textbf{0.63} & family-cluster GroupKFold & §5.10.5 & [OK] leakage-free \\
...
D1 & basin predictability acc(k=10), group-aware & \textbf{0.34} & family-cluster GroupKFold & §5.10.5 & [OK] leakage-free (~chance for 11 classes is 0.09; signal is real but weak) \\
...
D1 attractor classification & C1–C4 strong attractor (formal); \textbf{attractor-like dialog regime in practice} & 4/4 PASS on operational criteria, BUT: group-aware basin predictability acc(k=10) = 0.34 (well below the τ\_acc = 0.70 threshold under leakage-free CV); switching is granularity-sensitive (§5.10.7); semantic inspection (§5.10.8) finds dialogue-state / recent-context capture rather than a stable stylistic basin; neutral switching exceeds adversarial in the pilot. We retain D1 in the diagnostic taxonomy as an \textit{attractor-like dialog regime} but do not claim full strong-attractor status under group-aware criteria. & §3.1.1.5 + §5.10.5 + §5.10.7 + §5.10.8 & [!] caveat-required &  \\
\bottomrule
\end{tabularx}
\end{table}
...
> \textbf{Caveat (added in revision).} The T-sweep cells in this section
...
described in \texttt{paper/ed50\_rerun\_plan.md} was committed as a frozen
...
\textbf{~36–52-token range} — substantially below the sparse-data
...
> \textbf{Caveat (added in revision).} The geometric $V^\star$ values
...
\subsection{Group-aware basin-predictability: how much leakage is in the headline number?}
...
\subsection{Cluster-stability check: do the basins survive method changes?}
...
\subsection{Multi-granularity switching: does the dose-response survive different cluster definitions?}
...
\textbf{Combined with §5.10.5 and §5.10.6 results}, the picture is:
\begin{itemize}
...
\subsection{Per-cluster semantic inspection: what do the basins actually contain?}
...
and having a separate LLM (\texttt{gpt-5.5} with high reasoning effort)
...
\subsection{V* parameter-grid sensitivity (review weakness \#5 follow-up)}
...
The reviewer (\texttt{paper/openai\_review.md} Weakness \#3 and round-2
followup) observed that the original O2/O3 perturbation result —
94–100\% switching at all probed doses — is partly tautological,
because the replace-mode operator in \texttt{adversarial\_doseN}
\textit{overwrites} step 15's output entirely. Under that intervention,
...
\textbf{O3 results} (summarize + negate, replace operator) — added per
...
\subsection{Cross-metric correlations: do the regime diagnostics agree?}

> \textbf{Caveat (added in revision).} The correlations reported in this
...
\subsection{Embedding-space invariance: do the regimes survive a different embedder?}
...
\subsection{Cross-model thesis verification: do the headline claims survive on a smaller generator?}
...
cluster semantic inspection in §5.10.8 shows that \textbf{only O1's label
...
\textbf{In that sense, the barrier-endpoint structure (raw / net / persistent-escape, §3.1.1bis) is the operational bridge between a
...
canonical layout: \texttt{src/} (analysis + api + core + experiments +
...
\textbf{Lemma 1} (statement in §3.1.2).
...
\textbf{Insert vs overwrite modes} (added in revision; see review §3 and
\texttt{paper/ed50\_rerun\_plan.md}). The current \texttt{adversarial\_doseN}
condition implements \textit{state overwrite} (the operator output for
...
\subsection{Phase-0 pilot validation (moved from §5.1)}
...
\subsection{Phase-1 small-N taxonomy (moved from §5.2)}
...
\texttt{metrics/bootstrap\_summary.csv}; basin-predictability from §5.10.5;
...
Source: §5.10.5, \texttt{data/aggregated/group\_aware\_basin\_pred.csv}.
...
§3.1.1.5 — "max(z\_R, z\_D) ≥ 2 AND d ≥ 0.5" — therefore PASSES on
...
\subsection{Geometric V* and RG dendrogram per-regime tables (moved from §5.10)}
...
\begin{enumerate}
  ...
  \item \textbf{Run paired controls.} For each task, run the same loop multiple times \textit{without} perturbation. The disagreement rate between paired controls is your stochastic floor (§4.7, §5.10.5). Report it; this is what every later "switching rate" must be calibrated against.
  ...
  \item \textbf{Separate overwrite-style interventions from genuine perturbation response.} If your update rule replaces state with a generated summary, run an \textit{insert-mode} probe (§5.10.11): inject the same content as a non-replacing addition to context. The gap between overwrite and insert is the operator-overwrite contribution. If it dominates, your "fragility" measurement is partly a statement about your memory policy, not your generator.

  \item \textbf{Pre-register the equivalence rule and the analysis plan.} Persistence and net-effect estimates are sensitive to clustering granularity (§5.10.6, §5.10.7) and to the choice of equivalence rule. Pre-registering protects the report from inadvertent post-hoc tuning of the threshold.
\end{enumerate}
...
\texttt{paper/STRUCTURE\_REORG\_PROPOSAL.md}):
...
\begin{itemize}
  ...
  \item §3.1.3 effective-context-share formulation and geometric refinement (theory deep details).
  \item §5.4 Temperature-sweep extended tables, §5.11 cross-metric
