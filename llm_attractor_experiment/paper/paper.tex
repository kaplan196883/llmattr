...
geometric/behavioral triangulation & n/a & \textbf{rank-stable V* ordering across 6 inter-basin geodesics matches perturbation-derived dose-rankings (89–98\% of parameter combinations; absolute V* magnitudes are descriptive only, CV 14–24\% across a 45-point parameter grid)} (§5.12) \\
...
\begin{itemize}
  ...
  (sharpness dim, in particular, is \textit{not} embedder-invariant per §5.22).
...
\midrule
\textbf{O1 contractive} & PASS: $A^{\mathrm{final}}=0.85$ (§5.5) & PASS by dwell / basin-null gate; raw recurrence low, $R=0.29$ (§5.20) & PASS: recurrence remains low across embedders, $0.289/0.304/0.096$ (§5.22) & PASS: $\lambda_1^{\mathrm{late}}\approx 0.008\le 0.015$; $SD=1.70$ (§5.2, §5.21) & adversarial switch $54\%$; OOD floor $18$–$24\%$ (§5.7–§5.8) & \textbf{attractor (strong)} \\
\textbf{O2 oscillatory} & PASS: $A^{\mathrm{final}}=0.91$ (§5.5) & PASS: high recurrence / period-2 structure, $R=0.875$ (§5.20, §5.22) & PASS: high recurrence under all embedders, $0.875/0.711/0.783$ (§5.22) & PASS: period-2 re-entry; $SD=1.39$ (§5.2) & adversarial switch $94\%$ (§5.7) & \textbf{attractor (strong)} \\
\textbf{O3 absorbing} & PASS: $A^{\mathrm{final}}=0.93$ (§5.5) & PASS: trivial high recurrence, $R=0.924$ (§5.20, §5.22) & PASS: high recurrence under all embedders, $0.924/0.850/0.862$ (§5.22) & PASS: absorbing-collapse gate, $R\ge 0.90$, $SD=1.45\le 1.50$ (§5.2, §5.22) & adversarial switch $96\%$ (§5.7) & \textbf{attractor (strong)} \\
\textbf{D1 dialogue-state multi-basin} & PASS stratified: $A^{\mathrm{final}}=0.77$ (§5.5); \textbf{FAIL group-aware: 0.34} (§5.13) & PASS by dwell / basin-null gate; raw recurrence low, $R=0.21$ (§5.20) & PASS: recurrence remains low across embedders, $0.210/0.337/0.226$ (§5.22) & PASS: $\lambda_1^{\mathrm{late}}\approx 0.011\le 0.015$; $SD=1.89$ (§5.2, §5.21) & adversarial switch $60\%$; neutral $76\%$ (§5.7) & \textbf{attractor-like dialog regime} (formal C1–C4 PASS; group-aware C1 fails) \\
\textbf{D2 drill-down} & NT / FAIL: no publication-scale final basin-predictability cell; exploratory $A(k=5)=0.20$ with $n=25$ (§5.2) & NT / FAIL: no null-calibrated recurrence / dwell test at publication scale (§5.2, §7.7) & PARTIAL only: recurrence low across embedders, $0.296/0.176/0.073$, but small-$N$ (§5.22) & NT / FAIL: Lyapunov / exit-return not measured; $SD$ unavailable (§5.2) & adversarial switch $64\%$ (§5.10) & \textbf{not attractor} under this definition; exploratory regime candidate \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{Proof.} Take $m = m_{1/2}$ in Lemma 1; the bounds follow directly. Full proof in §13.1. $\square$
...
\textbf{Proof.} Take $m = 1$ in Lemma 1. $\square$ (Full step-by-step proofs of Corollaries 1 and 2 in §13.1.)
...
\textbf{Empirical verification.} §5.7 reports 94–96\% switching for O2
...
rendering via \texttt{ProcessPoolExecutor}) are in §13.10.
...
\subsection{Unified primary-results table}
...
\midrule
O1 & basin predictability acc(k=10), stratified & \textbf{0.80} & n=1350, 5-fold CV & §5.5, \texttt{basin\_pred.csv} & [!] \textbf{inflated by family leakage; group-aware = 0.73} \\
O1 & basin predictability acc(k=10), group-aware & \textbf{0.73} & family-cluster GroupKFold & §5.13 (this revision) & [OK] leakage-free \\
...
O1 adv & switching @ dose 200 (dense) & \textbf{0.620} & Wilson [0.55, 0.68], n=200 & §5.8.1 & [OK] dense rerun \\
O1 adv & switching @ dose 400 (dense) & \textbf{0.670} & Wilson [0.60, 0.73], n=200 & §5.8.1 & [OK] dense rerun \\
O1 adv & $\mathrm{ED50}_{\mathrm{raw}}$ (dense) & \textbf{36–52 tok} & 4PL=36; GLMM=41; family-cluster bootstrap median=52, 95\% CI [8.5, 242] & §5.8.1 / §3.1.2 & [OK] established \\
O1 adv & upper asymptote (dense 4PL) & \textbf{$a = 0.69$} & non-switching subpopulation ~31\% & §5.8.1 & [OK] \\
O1 adv & natural floor (control-vs-control, dense) & \textbf{0.347} & [0.310, 0.386], n=600 paired comparisons & §5.8.1 & [OK] established \\
...
O1 adv & $\mathrm{ED50}_{\mathrm{persist}}$ (dense, kicked-AND-persisted) & \textbf{undefined} & max persistent escape = 16\% at dose 400 & §5.17 (dense) / §3.1.2 & not reached in tested range \\
...
O2 & basin predictability acc(k=10), stratified & \textbf{0.90} & n=1350, 5-fold CV & §5.5 & [!] \textbf{inflated by family leakage; group-aware = 0.60} \\
O2 & basin predictability acc(k=10), group-aware & \textbf{0.60} & family-cluster GroupKFold & §5.13 & [OK] leakage-free \\
...
O3 & basin predictability acc(k=10), stratified & \textbf{0.91} & n=1350, 5-fold CV & §5.5 & [!] \textbf{inflated by family leakage; group-aware = 0.63} \\
O3 & basin predictability acc(k=10), group-aware & \textbf{0.63} & family-cluster GroupKFold & §5.13 & [OK] leakage-free \\
...
D1 & basin predictability acc(k=10), stratified & 0.60 & n=450, 5-fold CV & §5.5 & [!] \textbf{inflated by family leakage; group-aware = 0.34} \\
D1 & basin predictability acc(k=10), group-aware & \textbf{0.34} & family-cluster GroupKFold & §5.13 & [OK] leakage-free (~chance for 11 classes is 0.09; signal is real but weak) \\
...
D1 attractor classification & C1–C4 strong attractor (formal); \textbf{attractor-like dialog regime in practice} & 4/4 PASS on operational criteria, BUT: group-aware basin predictability acc(k=10) = 0.34 (well below the τ\_acc = 0.70 threshold under leakage-free CV); switching is granularity-sensitive (§5.15); semantic inspection (§5.16) finds dialogue-state / recent-context capture rather than a stable stylistic basin; neutral switching exceeds adversarial in the pilot. We retain D1 in the diagnostic taxonomy as an \textit{attractor-like dialog regime} but do not claim full strong-attractor status under group-aware criteria. & §3.1.3 + §5.13 + §5.15 + §5.16 & [!] caveat-required &  \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{The four (plus one) regimes at a glance}
...
\subsection{Phase 0 — pilot validation (one-line)}
...
basin profile that became O1. \textbf{Full pilot history moved to §13.8};
the publication-scale story below subsumes these pilot findings.

\subsection{Phase 1 — small-N taxonomy (one-line)}
...
\subsection{Phase 2 — publication-scale verification}
...
\subsection{Phase 2b — temperature sensitivity}
...
\subsection{Phase 3a — perturbation pilots}
...
\subsection{Phase 3b — dose-response}
...
\subsubsection{Confirmatory dense-dose rerun (n=200/cell, 8 doses)}
...
\subsection{Phase 3c — injection-time sweep}
...
\subsection{Phase 3d — drill-down dialog (D2)}
...
\textbf{What this changes in the paper.} The §5.5 headline numbers should
...
\textbf{Combined with §5.13 and §5.14 results}, the picture is:
\begin{itemize}
...
(§5.8, Figure 4). We asked: is this \textbf{one population at half-mixing}
...
doses; same \texttt{joint\_pca10\_clusters.csv} as §5.8.1):
\begin{table}[h!]
\centering
\small
...
dose 400} (§5.8.1, dense): the gap is \textbf{51 percentage points}.
...
\textbf{What this changes in the paper.} §5.12's V\textit{ tables retain their
...
§5.6) emerges \textit{only when one measures the cost of nudging}. Bulk
...
\subsection{Regimes are properties of nudges, not prompts alone}
...
\toprule
 & dialog architecture \\
\midrule
\textbf{free} (curious user / helpful agent) & D1 dialogue-state-driven multi-basin \\
...
The within-vendor cross-generation replication on \texttt{gpt-4.1-nano}
...
In the \textbf{two OpenAI generators} tested here, the qualitative
taxonomy is preserved across both, while the \textit{barrier magnitude
...
\textbf{A note on regime labels.} The labels "append-contractive,
...
cluster semantic inspection in §5.16 shows that \textbf{only O1's label
...
\subsection{Why append resists and replace yields}
...
the original basin. \textbf{Lemma 1 (§3.1.4) formalizes this asymmetry by
...
\subsection{D1 and D2 reveal two kinds of dialog attractor}
...
\subsection{Barrier height is the missing dimension of regime analysis}
...
\textbf{Engineering interpretation of the three endpoints.}

\begin{itemize}
  \item \textit{Raw switching} asks whether the system moved.
  \item \textit{Net switching} asks whether it moved more than ordinary stochastic drift.
  \item \textit{Persistent escape} asks whether it committed after recovery turns.
\end{itemize}
...
\subsection{Why the geometric picture matters}
...
partly a problem of \textbf{nudge engineering}. The table below summarizes
...
The perturbation-barrier protocol \textit{could} in principle be read as a
...
to another. \textbf{However, we have not run experiments specifically
...
right \textit{baseline framework}: any robustness claim is meaningful only
...
\textbf{In that sense, the barrier-endpoint structure (raw / net / persistent-escape, §3.1.2) is the operational bridge between a
...
into MP4 via \texttt{imageio-ffmpeg}. Implementation details in §13.5.
...
MiniMax \texttt{MiniMax-Text-01}. Full version table in §13.7.
...
\subsection{Phase-0 pilot validation (moved from §5.3)}
...
\subsection{Phase-1 small-N taxonomy (moved from §5.4)}
...
\texttt{metrics/bootstrap\_summary.csv}; basin-predictability from §5.13;
...
Source: §5.13, \texttt{data/aggregated/group\_aware\_basin\_pred.csv}.
...
\subsection{Geometric V* and RG dendrogram per-regime tables (moved from §5.12)}
...
\begin{enumerate}
  ...
  \item \textbf{Run paired controls.} For each task, run the same loop multiple times \textit{without} perturbation. The disagreement rate between paired controls is your stochastic floor (§4.7, §5.13). Report it; this is what every later "switching rate" must be calibrated against.
  ...
  \item \textbf{Separate overwrite-style interventions from genuine perturbation response.} If your update rule replaces state with a generated summary, run an \textit{insert-mode} probe (§5.19): inject the same content as a non-replacing addition to context. The gap between overwrite and insert is the operator-overwrite contribution. If it dominates, your "fragility" measurement is partly a statement about your memory policy, not your generator.

  \item \textbf{Pre-register the equivalence rule and the analysis plan.} Persistence and net-effect estimates are sensitive to clustering granularity (§5.14, §5.15) and to the choice of equivalence rule. Pre-registering protects the report from inadvertent post-hoc tuning of the threshold.
\end{enumerate}
...
\begin{itemize}
  \item §5.11 Cross-experiment aggregation (pipeline-script documentation).
  ...
  \item §5.6 Temperature-sweep extended tables, §5.20 cross-metric
