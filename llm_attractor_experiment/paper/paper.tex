...
\caption{Fig 1. \textbf{Headline perturbation dose response.} Summary dose-response view for recursive-loop perturbations, emphasizing that raw switching, stochastic floor, and persistent escape are distinct endpoints. The figure orients the reader before the formal endpoint definitions. Source: \texttt{data/aggregated/perturbation\_dose\_response/dose\_response.png}.\protect\footnote{Fig 1 plots fraction switched at step 29 against injected token dose on a log scale, with 50 trajectories per cell, points as raw proportions, whiskers as binomial uncertainty, and the dotted line marking the 0.50 switching threshold. The intended comparison is not primarily across colors at a fixed dose, but within the same O1 continue loop across content types: neutral perturbations stay at 0.22, 0.26, 0.24, and 0.24 from doses 20 to 400, while adversarial in-distribution perturbations rise from 0.26 at dose 20 to 0.34 at 80 and 0.54 raw switch at dose 200 before settling at 0.48 at 400. The non-obvious feature is that D1 dialog neutral is already saturated, 0.62 at dose 5 and 0.70 to 0.78 thereafter, so its apparent fragility is not a dose response in the relevant sense, whereas O1 continue separates out-of-distribution drift from content-matched accumulation. This is the first evidence for the paper's central claim: recursive-loop stability is jointly set by model, memory policy, perturbation content, and the persistence criterion, not by dose alone. Read against the paper-wide estimates, the O1 adversarial curve is compatible with ED50 raw about 40 tokens, a stochastic floor about 35 percent from control vs control drift, and a raw plateau near 67 percent, but Fig 1 also warns that raw switching is not durable escape: net switching never reaches plus 50 percentage points and persistent escape is at most 16 percent at dose 400 under K-means k equals 12. The figure licenses treating in-distribution dose sensitivity as a diagnostic of context-averaging architectures. It would have been falsified if O1 neutral perturbations showed the same graded rise, or if adversarial O1 failed to separate from the natural drift floor.}}
\end{figure}
\end{savenotes}
