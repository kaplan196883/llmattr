...
\caption{\textbf{Original stratified basin-predictability curves.} Stratified-CV basin-predictability curves are retained for audit but should be interpreted alongside the leakage-aware GroupKFold results in main-text Fig 5. Source: \texttt{data/aggregated/basin\_predictability\_cross/cross\_basin\_predictability.png}.\\[2pt]{\footnotesize\itshape ED Fig 6 shows the original stratified cross-validation basin-predictability curves, with accuracy for predicting the final cluster plotted against the predictor step k for each regime, and the companion bar plot contrasting seed-deterministic step 0 performance with the recursive endpoint. Visually, O2 and O3 appear almost saturated, O1 rises more gradually, and D1 improves but remains substantially lower. The correct reading is that these high-accuracy curves are an optimistic diagnostic upper bound, not a valid estimate of cross-family generalization, and they should be interpreted only alongside the group-aware Fig 5 in the main text. At k=10 under stratified splitting, the apparent accuracies are O1 0.803, O2 0.896, O3 0.912, and D1 0.604, which look like strong evidence that basin membership is largely predictable from early trajectory state. After GroupKFold correction, however, the same comparison falls to O1 0.732, O2 0.596, O3 0.629, and D1 0.336, corresponding to drops of about 7, 30, 28, and 27 percentage points. The especially important result is that the near-ceiling O2 and O3 stratified accuracies were dominated by family-fingerprint leakage rather than robust cross-family basin information. We retain this panel as an audit artifact precisely because it explains why the leakage correction was necessary. It should not be used as primary evidence for basin generalization; the stricter group-aware Fig 5 is the load-bearing result. Without this audit panel, the original stratified numbers could easily be mistaken for a substantive generalization claim.}}
\end{figure}
\end{savenotes}
