...
"claim / measured / flag" with `✓` (within tolerance) or `✗` (Δ outside
tolerance + the delta).

**Current status (commit-time)**: 103 cells verified, 94 pass
(91.3%), 8 fail (7.8%). All 8 failing cells are concentrated in
**§5.4 T-sweep `acc(k=5)` table** — a material discrepancy where the
article claims a clean monotonic O1 decay (0.85 → 0.55) and a flat D1
trace (0.88 → 0.83), but the current per-experiment
`basin_predictability.csv` outputs give O1 ≈ 0.62–0.70 (no clear
monotone) and D1 ≈ 0.40–0.53. See "Anomalies" section in `RESULTS.md`
for the three candidate explanations and the recommended action.

§5.0 / §5.3 / §5.5 / §5.6 / §5.7 / §5.10 all reproduce within
tolerance. Re-run `python -m scripts.publication_summary` after any
code/data change to keep this verification current.
